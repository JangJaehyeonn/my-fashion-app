...
> 기획 · 백엔드 · AI 서버 · Android · 인프라 전체를 혼자 구현한 1인 개발 프로젝트입니다 (포트폴리오 + 실서비스 출시 목표).
...
처음에는 "개인 옷장 관리 + 가상 피팅" 형태로 개발을 시작했습니다. 하지만 사용자가 자기 옷을 일일이 사진 찍어 등록해야만 추천을 받을 수 있는 구조 자체가, 정작 목표 사용자층인 "패션에 무지한 사람들"에게는 가장 큰 진입장벽이라고 판단했습니다.

2026년 7월, 옷장 등록/관리와 코디 캘린더 기능을 걷어내고 등록 없이 날씨·상황·체형 정보만으로 바로 추천을 받는 현재의 3대 기능(오늘의 코디 추천 / 내 옷 진단 / 쇼핑 도우미) 중심으로 서비스를 전면 재설계했습니다. 기존에 구현해둔 소셜 로그인, 체형/취향 프로필, 날씨 API 연동, EC2/Docker/CI-CD 인프라는 그대로 재사용해 재설계에 드는 리스크와 시간을 줄였습니다.
...
| 인프라 | AWS EC2 + Docker + GitHub Actions + DuckDNS(동적 도메인, systemd로 부팅 시 IP 자동 갱신) |
...
[DuckDNS] ◀── AWS EC2 (systemd, 부팅 시 퍼블릭 IP 자동 갱신)
```
